...
class Tapper:
    ...
    async def process_auth_data(self, data: dict):
        ...
        self.friends_count = data.get("friends", 0)
    ...
    async def prioritize_upgrades(self, user_data: dict) -> list:
        ...
        for u in available_upgrades:
            ...
            if upgrade_id == 'maxEnergy' and level >= settings.MAX_LEVEL_ENERGY:
                continue

            if upgrade_id == 'coinsPerTap' and level >= settings.MAX_LEVEL_TAP:
                continue

            if level >= settings.MAX_LEVEL_PROMO:
                continue
